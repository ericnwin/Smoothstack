...
extracted_data_of_order_price_quantity = map(
    lambda x: (x[0], x[2] * x[3]), book_orders)

# Editing list to account for orders below min_order
totals = list(map(lambda x: (x[0], x[1] + 10) if (x[1] <
              min_order) else x, extracted_data_of_order_price_quantity))
print(totals)
